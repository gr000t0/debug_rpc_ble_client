...
function do_test {
    ...
    if [ -z "$STDOUT_CONTENT" ] && [ -z "$STDERR_CONTENT" ]; then
        echo "<p style='color:green;'>OK! (empty Response)</p>" >> $REPORT_FILE
    fi
...
sleep 0.5
stty sane
read -p "Turn TouchControl on, bring any Zone to any Powerlevel and press Enter to continue..." -n 1 -s
echo -e "\n"
...
do_test "extractorService" "setExtractorMode" "1"
...
sleep 0.5
stty sane
read -p "Turn off Touchcontrol and press any key to continue..." -n 1 -s
echo -e "\n"

do_test "cooktopService" "setChildLock" "on"

do_test "cooktopService" "setChildLock" "off"

do_test "cooktopService" "setSignalVolume" "5"

sleep 0.5
stty sane
read -p "Done! Please turn on TouchCOntrol and press any key to continue..." -n 1 -s
echo -e "\n"
...
sleep 3 
do_test "wifiProvisioningService" "getWifiStatus"

echo "<h1 style='color:#ff8b00;'>ZoneService-Setter</h1>" >> $REPORT_FILE
echo -e "\nTesting ZoneService-Set-Calls...\n"
echo "Testing setMode front_right-6"
echo "Please confirm on TouchControl ..."
do_test "zoneService" "setMode" "front_right-6"


echo "Testing setMode back_right-3"
echo "Please confirm on TouchControl ..."
do_test "zoneService" "setMode" "back_right-3"



do_test "zoneService" "setTimer" "front_right-30000"


echo "Testing setBridged front_right-back_right"
echo "Please confirm on TouchControl ..."
do_test "zoneService" "setBridged" "front_right-back_right"


echo "Testing startOrdModifyCsf"
echo "Please confirm on TouchControl ..."
do_test "zoneService" "startOrModifyCsf" "front_left-11-3-pasta-100-10-30000-0-false-60-140-1"
do_test "zoneService" "getZoneSettings" "front_left"

echo -e "\nDone!\n"

echo "<h1 style='color:#ff8b00;'>Testing Restart-Cm</h1>" >> $REPORT_FILE

echo "Restarting CM Please check on debug-output..."
do_test "cooktopService" "restartConnectivityModule"
